...
import { Zap, Wand2, Search, ChevronDown, Check, Cpu, LayoutGrid } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function ModelSelector({
  ...
  const setOpen = useCallback((value: boolean | ((prev: boolean) => boolean)) => {
    const nextValue = typeof value === 'function' ? value(controlledOpen ?? internalOpen) : value;
  ...
  // Sort: new models first, then alphabetical
  const sortedModelsForList = useMemo(() => {
    if (filteredByProvider.length === 0) return filteredByProvider;
    const newModels: Array<(typeof models)[0]> = [];
    const regularModels: Array<(typeof models)[0]> = [];
    for (const model of filteredByProvider) {
      if (model.isNew) {
  ...
  const handleModelListKeyDown = useCallback(
    ...
    [displayModels, focusedIndex, onModelChange, activeProviders, selectedProvider],
  );
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {model.experimental && (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-amber-500/10 text-amber-600 dark:text-amber-400 leading-none">
                EXPERIMENTAL
              </span>
            )}
          </div>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className={cn('p-0.5 rounded', isMobile && 'p-1')}>
                    <Zap
                      className={cn('text-amber-500/70', isMobile ? 'size-3' : 'size-2.5')}
                    />
                  </div>
                </TooltipTrigger>
  ...
  const currentModelProvider =
    ...
    (currentModel ? getModelProvider(currentModel.value, currentModel.label) : undefined);

  const TriggerButton = React.forwardRef<
    React.ComponentRef<typeof Button>,
    React.ComponentPropsWithoutRef<typeof Button>
  >((props, ref) => {
    // If children (custom trigger) are provided, render them
    if (children) {
      return (
        <span ref={ref as React.Ref<HTMLSpanElement>} {...(props as Record<string, unknown>)}>
          {children}
        </span>
      );
    }
