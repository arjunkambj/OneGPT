...
- In Next.js Server Components, Server Actions, and Route Handlers, use the helpers from `convex/nextjs` instead of `ConvexReactClient`:
  - Use `preloadQuery` in a Server Component when a Client Component should receive preloaded data and stay reactive via `usePreloadedQuery`.
...
- `preloadQuery` uses `cache: "no-store"`, so any Server Component using it will not be statically rendered.
- Avoid multiple `preloadQuery` calls on the same page when the UI depends on a single consistent snapshot. `convex/nextjs` server helpers use the stateless HTTP client, so separate calls are not guaranteed to read the exact same database state.
- Server-side `convex/nextjs` helpers require `NEXT_PUBLIC_CONVEX_URL` to be configured unless you explicitly pass a `url` option.
